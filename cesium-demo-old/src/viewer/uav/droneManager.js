 ...
 */
export function animateDroneAlongPath(viewer, droneEntity, path, nodeMap) {
    if (!droneEntity || !path || path.length === 0) {
    ...
    const speed = 10; // đơn vị tốc độ, có thể tùy chỉnh
  
    for (let i = 0; i < path.length; i++) {
      const node = nodeMap[path[i]];
      const pos = Cesium.Cartesian3.fromDegrees(node.lng, node.lat, node.alt);
      property.addSample(time, pos);
  
      // tăng thời gian theo khoảng cách
      if (i < path.length - 1) {
        const nextNode = nodeMap[path[i + 1]];
        const dist = Math.sqrt(
          (node.lng - nextNode.lng) ** 2 +
          (node.lat - nextNode.lat) ** 2 +
          (node.alt - nextNode.alt) ** 2
        );
        time = Cesium.JulianDate.addSeconds(time, dist * speed, new Cesium.JulianDate());
      }
    }
