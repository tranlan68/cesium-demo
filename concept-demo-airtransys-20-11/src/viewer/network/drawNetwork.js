  ...
  nodes.forEach((node) => {
    ...
    if (node.id === "P1" || node.id === "P5") {
      ...
      let edgeEntity = viewer.entities.add({
        ...
        // },
        position: Cesium.Cartesian3.fromDegrees(node.lng, node.lat, 12.5), // 5 = center
        cylinder: {
          length: 25,
          topRadius: 11 * 1.5,
          bottomRadius: 11 * 1.5,
...
function drawRoutes(viewer, routes, radius = 150) {
  ...
  routes.forEach((route) => {
    let positions = route.path.map((p) =>
      Cesium.Cartesian3.fromDegrees(p.lng, p.lat, 20 - 5)
    );
    let edgeEntity = viewer.entities.add({
      polylineVolume: {
        positions,
...
function createCircle3D(center, next, radius = 1.5, segments = 16) {
  let circle = [];
  let forward = Cesium.Cartesian3.subtract(
    next,
    center,
